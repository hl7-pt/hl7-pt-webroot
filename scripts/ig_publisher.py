...
class ReleasePublisher:
    def __init__(
        ...
        self.webroot_repo = (
            webroot_repo or "https://github.com/WorldHealthOrganization/smart-html"
        )
        self.webroot_branch = webroot_branch
        self.history_repo = (
            history_repo or "https://github.com/HL7/fhir-ig-history-template"
        )
    ...
    def log_disk_usage(self, label: str = ""):
        ...
        lines = [
            header,
            f"   💾 Free disk space: {free_mb:,.0f} MB ({free_mb / 1024:,.1f} GB)",
        ]
    ...
    def _safe_checkout(self, repo: git.Repo, branch: str):
        ...
        try:
            ...
            if remote_ref in remotes:
                ...
                self.log_progress(
                    f"Branch '{branch}' not found (local or remote); staying on current branch."
                )
    ...
    def clone_repo(self, url, path, branch=None, use_sparse=False, sparse_dirs=None):
        ...
        ensured = _normalize_sparse_list(
            (sparse_dirs or []) + (ALWAYS_INCLUDE if use_sparse else [])
        )
        ...
        if os.path.exists(path) and os.path.isdir(os.path.join(path, ".git")):
            ...
            try:
                ...
                if use_sparse and ensured:
                    needs_no_cone = any(_looks_like_file(p) for p in ensured) or any(
                        "*" in p for p in ensured
                    )
    ...
    def has_changes(self, repo_path):
        ...
        try:
            ...
            self.log_progress(
                f"Warning: Could not check repository status for {repo_path}: {e}"
            )
            return False
    ...
    def create_branch_and_commit(self, repo_path, branch_name, commit_message):
        ...
        try:
            ...
            if is_sparse:
                self.log_progress(
                    "Using --sparse flag for git add (sparse checkout detected)"
                )
            ...
            if e.stderr:
                self.log_progress(
                    f"Error details: {e.stderr.decode('utf-8', errors='ignore')}"
                )
            return False
        except Exception as e:
            self.log_progress(f"Error creating branch and committing: {e}")
    ...
    def create_github_pr(self, repo_url, head_branch, base_branch, title, body):
        ...
        if not self.github_token:
            if self.is_github_actions:
                self.log_progress(
                    "⚠️ No GitHub token available in GitHub Actions. Ensure GITHUB_TOKEN is properly configured."
                )
            else:
        ...
        try:
            ...
            if self.is_github_actions and is_same_repo:
                self.log_progress(
                    f"📝 Creating PR in same repository ({current_repo}) using GITHUB_TOKEN"
                )
    ...
    def create_prs_if_needed(self):
        ...
        if self.has_changes(self.webroot_dir):
            ...
            if self.create_branch_and_commit(
                self.webroot_dir, branch_name, commit_message
            ):
                pr_title = (
                    f"FHIR IG Content Update - {datetime.now().strftime('%Y-%m-%d')}"
                )
...
"""
        ...
        if self.has_changes(self.registry_dir):
            ...
            commit_message = (
                f"Update IG registry - {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
            )
            if self.create_branch_and_commit(
                self.registry_dir, branch_name, commit_message
            ):
...
"""
        ...
        try:
            ...
            self.run_command(
                [
                    ...
                    publisher_jar_path,
                    "-ig",
                    ".",  # Current directory (which is now source_dir)
    ...
    def publish(self):
        ...
        registry_path = os.path.abspath(
            os.path.join(self.registry_dir, "fhir-ig-list.json")
        )
        ...
        self.run_command(
            [
                ...
                package_cache_path,
                "-source",
                source_path,  # Use absolute path instead of '.'
                "-web",
                webroot_path,
                "-temp",
                temp_path,
    ...
    def prepare(self):
        ...
        if slug:
            ...
            self.log_progress(
                "⚠️ Could not detect IG slug from publication-request.json"
            )
        ...
        if self.enable_sparse_checkout:
            ...
            sparse_dirs_for_webroot = sorted(
                set(_normalize_sparse_list(sparse_dirs_for_webroot))
            )
            self.log_progress(
                f"➕ Sparse includes: {' '.join(sparse_dirs_for_webroot)}"
            )
        ...
        if self.enable_sparse_checkout and slug:
            ...
            if not os.path.exists(pkg):
                self.log_progress(
                    f"⚠️ Expected '{pkg}' not found in webroot after sparse checkout."
                )
    ...
    def _ensure_gh_pages_checkout(self, workdir):
        ...
        if not remote:
            raise RuntimeError(
                "GITHUB_REPOSITORY or token missing; cannot push to gh-pages"
            )
    ...
    def push_sitepreview_to_gh_pages(self):
        ...
        if not remote:
            self.log_progress(
                "Skipping gh-pages push (no GITHUB_TOKEN or GITHUB_REPOSITORY)."
            )
            return
        ...
        self.run_command(
            [
                "bash",
                "-lc",
                f'cd "{ghdir}" && git rm -r --cached {self.sitepreview_dir}/ig-build-zips || true',
            ]
        )
            ...
            for fname in filenames:
                ...
                try:
                    if os.path.getsize(fpath) > limit_bytes:
                        ...
                        self.log_progress(
                            f"Moved large file ({size_mb:.1f} MB): {rel} -> release-assets/"
                        )
                        moved.append(rel)
                except OSError:
                    pass
    ...
    def _upload_release_assets(self):
        ...
        try:
            ...
            if resp.status_code != 200:
                self.log_progress(
                    f"No release found for tag {tag}, skipping asset upload"
                )
                return
            ...
            for fname in os.listdir(release_assets_dir):
                ...
                self.log_progress(
                    f"Uploading release asset: {fname} ({size_mb:.1f} MB)"
                )
                ...
                if upload_resp.status_code in (200, 201):
                    ...
                    self.log_progress(
                        f"Failed to upload {fname}: {upload_resp.status_code}"
                    )
        except Exception as e:
    ...
    class CustomCheckbox(tk.Canvas):
        ...
        def setup_widget(self):
            ...
            if self.text:
                ...
                bbox = self.bbox("all")
                if bbox:
                    self.configure(width=bbox[2] + padding, height=bbox[3] + padding)
            else:
                self.configure(
                    width=box_size + padding * 2, height=box_size + padding * 2
                )
        ...
        def on_enter(self, event):
            ...
            if not self.variable.get():
                self.itemconfig(
                    self.checkbox_id,
                    fill="#F0F0F0" if self.bg == "#FFFFFF" else "#3A4356",
                )
...
def main():
    ...
    parser.add_argument(
        "--gui", action="store_true", help="Launch beautiful GUI interface"
    )
    parser.add_argument("--source", type=str, help="Path to the IG source folder")
    parser.add_argument(
        "--source-repo", type=str, help="URL to the IG source repository"
    )
    ...
    parser.add_argument(
        "--sparse", nargs="*", help="Sparse checkout folders for webroot"
    )
    parser.add_argument(
        "--enable-sparse", action="store_true", help="Enable sparse checkout"
    )
    parser.add_argument("--registry-repo", type=str, help="Registry repo URL")

    # GitHub PR arguments
    parser.add_argument(
        "--enable-pr", action="store_true", help="Enable automatic PR creation"
    )
    parser.add_argument("--github-token", type=str, help="GitHub personal access token")
    parser.add_argument(
        "--webroot-pr-target", type=str, default="main", help="Webroot PR target branch"
    )
    parser.add_argument(
        "--registry-pr-target",
        type=str,
        default="master",
        help="Registry PR target branch",
    )
    parser.add_argument(
        "--global-config", type=str, help="Path to global default release-config.yaml"
    )
    parser.add_argument(
        "--local-config",
        type=str,
        default="release-config.yaml",
        help="Path to repo-specific release-config.yaml",
    )
    parser.add_argument(
        "--publish-gh-pages",
        action="store_true",
        help="After publishing, push site into gh-pages/<sitepreview-dir>",
    )
    parser.add_argument(
        "--sitepreview-dir",
        type=str,
        default="sitepreview",
        help="Subfolder in gh-pages to place the built site",
    )
    parser.add_argument(
        "--gh-pages-branch",
        type=str,
        default="gh-pages",
        help="Branch to publish the preview to",
    )
    parser.add_argument(
        "--exclude",
        action="append",
        default=[],
        help="Paths (relative to webroot) to exclude when copying to sitepreview; repeatable",
    )
    parser.add_argument(
        "--ensure-pubreq",
        action="store_true",
        help="Create minimal publication-request.json if missing",
    )
    ...
    if args.gui:
        ...
        publisher = ReleasePublisher(
            ...
            or config.get("enable_sparse_checkout", False),
            ...
            pubreq_overrides={
                ...
                "canonical": args.pubreq_canonical
                or os.environ.get("PUBREQ_CANONICAL"),
                "package_id": args.pubreq_package_id
                or os.environ.get("PUBREQ_PACKAGE_ID"),
